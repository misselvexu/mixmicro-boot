...
import xyz.vopen.framework.logging.client.admin.discovery.lb.support.DefaultLoadBalanceStrategy;
...
/**
 * Mixmicro Boot Logging Appoint Admin Discovery Support multiple admins
 *
...
public class LoggingAppointAdminDiscovery extends LoggingAbstractAdminDiscovery {
  ...
  /** Mixmicro Boot Logging Admin Server Address To Current Thread */
  ...
  /**
   * load balance lookup admin server address {@link LoadBalanceNode#getAddress()} get load-balanced
   * admin address {@link LoadBalanceStrategy#lookup(String[])} exclude basic auth info {@link
   * #extractBasicAuth(String)} and append {@link #HTTP_PREFIX}
   *
  ...
  public String lookup() throws MinBoxLoggingException {
    Assert.notNull(adminServerAddress, "Mixmicro Boot Logging Admin Server Address Is Null.");
  ...
  protected String extractBasicAuth(String adminServerAddress) {
    if (adminServerAddress.indexOf(BASIC_SPLIT) > 0) {
      adminServerAddress =
          adminServerAddress.substring(adminServerAddress.indexOf(BASIC_SPLIT) + 1);
    }
