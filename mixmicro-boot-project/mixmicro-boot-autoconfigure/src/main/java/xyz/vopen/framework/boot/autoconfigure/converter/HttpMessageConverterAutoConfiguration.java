...
package xyz.vopen.framework.boot.autoconfigure.converter;

import com.alibaba.fastjson.serializer.SerializerFeature;
import com.alibaba.fastjson.serializer.ValueFilter;
import com.alibaba.fastjson.support.config.FastJsonConfig;
import com.alibaba.fastjson.support.spring.FastJsonHttpMessageConverter;
import xyz.vopen.framework.boot.common.tools.ClassTools;
...
public class HttpMessageConverterAutoConfiguration {
  ...
  /**
   * http message converter fastjson实现实例 通过fastjson方式进行格式化返回json字符串
   *
  ...
  /**
   * 获取项目中定义的ValueFilter实现类列表 通过BeanFactory读取本项目的Base Package List
   *
            ...
            pack ->
                filterClass.addAll(
                    (Collection<? extends Class>)
                        ClassTools.getSubClassList(pack, ValueFilter.class)));
