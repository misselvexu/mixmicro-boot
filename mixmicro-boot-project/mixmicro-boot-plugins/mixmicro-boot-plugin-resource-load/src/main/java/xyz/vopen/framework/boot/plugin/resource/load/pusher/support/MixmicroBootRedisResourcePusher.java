 ...
 *     <p>DateTime：2019-04-19 09:34
 */
public class MixmicroBootRedisResourcePusher extends MixmicroBootJdbcResourcePusher {
  /** logger instance */
  static Logger logger = LoggerFactory.getLogger(MixmicroBootRedisResourcePusher.class);

  /** Redis Template */
  private RedisTemplate redisTemplate;

  public MixmicroBootRedisResourcePusher(
      MixmicroBootResourceStoreDelegate mixmicroBootResourceStoreDelegate, RedisTemplate redisTemplate) {
    super(mixmicroBootResourceStoreDelegate);
    this.redisTemplate = redisTemplate;
  }

  /**
   * Check whether the data is cached in redis If not cached, query from the database If cached ,
   * query from the redis
   *
  ...
  public List<String> loadResourceUrl(
    ...
    if (ObjectUtils.isEmpty(sourceFieldValue) || ObjectUtils.isEmpty(resourceType)) {
      logger.warn(
          "@ResourceField param [source]|[type]|[name] have empty value, load resource urls with redis fail.");
      return Collections.emptyList();
    }
    // formatter redis key
    String resourceRedisKey =
    ...
    if (resourceSize <= 0) {
      ...
      List<String> resourceUrls =
          super.loadResourceUrl(declaredMethod, sourceFieldValue, resourceType);
  ...
  public void deleteResourceUrl(
    ...
    if (ObjectUtils.isEmpty(sourceFieldValue) || ObjectUtils.isEmpty(resourceType)) {
      logger.warn(
          "@ResourceField param [source]|[type]|[name] have empty value, delete resource urls with redis fail.");
      return;
    }
    // formatter redis key
    String resourceRedisKey =
  ...
  public void insertResourceUrl(
    ...
    if (ObjectUtils.isEmpty(sourceFieldValue)
      ...
      logger.warn(
          "@ResourceField param [source]|[type]|[name] have empty value, insert resource urls with redis fail.");
      return;
    }
  ...
  public void updateResourceUrl(
    ...
    if (ObjectUtils.isEmpty(sourceFieldValue)
        ...
        || ObjectUtils.isEmpty(resourceUrls)) {
      logger.warn(
          "@ResourceField param [source]|[type]|[name] have empty value, update resource urls with redis fail.");
      return;
    }
