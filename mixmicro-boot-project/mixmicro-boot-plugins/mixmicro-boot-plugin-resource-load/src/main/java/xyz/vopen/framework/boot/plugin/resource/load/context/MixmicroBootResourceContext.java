...
package xyz.vopen.framework.boot.plugin.resource.load.context;
...
/**
 * Mixmicro Boot Resource Context
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 *     <p>DateTime：2019-04-15 09:57
 */
public class MixmicroBootResourceContext {
  /** logger instance */
  static Logger logger = LoggerFactory.getLogger(MixmicroBootResourceContext.class);

  /** key -> method name value -> declared resource field annotation list */
  private static ConcurrentMap<String, List<ResourceField>> RESOURCE_FIELD_ANNOTATIONS =
      new ConcurrentHashMap();
  /** key -> method name value -> field instance */
  ...
  public static Field getPushFieldFromCache(String declaredFieldClassName, String fieldName) {
    ...
    Field field = MixmicroBootResourceContext.RESOURCE_FIELDS.get(cacheKey);
    return field;
  }
   ...
   * @param fieldName field name
   */
  public static void setPushFieldToCache(
      String declaredFieldClassName, String fieldName, Field field) {
    String cacheKey = formatterCacheKey(declaredFieldClassName, fieldName);
    logger.debug("Cache method [{}] push field to memory", cacheKey);
    MixmicroBootResourceContext.RESOURCE_FIELDS.put(cacheKey, field);
  }
  ...
  public static List<ResourceField> getResourceFieldFromCache(Method method) {
    ...
    return MixmicroBootResourceContext.RESOURCE_FIELD_ANNOTATIONS.get(methodName);
  }
  ...
  public static void setResourceFieldToCache(Method method, List<ResourceField> resourceFields) {
    ...
    MixmicroBootResourceContext.RESOURCE_FIELD_ANNOTATIONS.put(methodName, resourceFields);
  }
   ...
   * @return redis key
   */
  public static String formatterCacheKey(
      Method method, String sourceFieldValue, String resourceType) {
    String expression = "%s.%s.%s";
    return String.format(
        expression, method.getDeclaringClass().getName(), resourceType, sourceFieldValue);
  }
}
