...
import xyz.vopen.framework.boot.common.tools.ListTools;
...
import xyz.vopen.framework.boot.plugin.resource.load.tools.ResourceFieldTools;
...
/**
 * Mixmicro Boot Resource Pusher Abstract Support Provide access to resource or source field
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 *     <p>DateTime：2019-04-19 09:40
 */
public abstract class MixmicroBootAbstractResourcePusher implements MixmicroBootResourcePusher {
   ...
   */
  public abstract List<String> loadResourceUrl(
   ...
   * @param resourceType resourceType
   */
  public abstract void deleteResourceUrl(
      Method declaredMethod, String sourceFieldValue, String resourceType);
   ...
   */
  public abstract void insertResourceUrl(
   ...
   * @param resourceUrls resource urls
   */
  public abstract void updateResourceUrl(
      Method declaredMethod,
      String sourceFieldValue,
      String resourceType,
      List<String> resourceUrls);
   ...
   */
  @Override
  public void insertResource(Method declaredMethod, Object[] param) throws MixmicroBootException {
        ...
        .forEach(
            resourceField -> {
              ...
              Object sourceFieldValue =
                  ResourceFieldTools.getMatchSourceValue(resourceField.source(), param);
              // get expression match resource field value
              Object resourceFieldValue =
                  ResourceFieldTools.getMatchResourceValue(resourceField.name(), param);

              // if have value
              if (!ObjectUtils.isEmpty(sourceFieldValue)
                  && !ObjectUtils.isEmpty(resourceFieldValue)) {
                List<String> resourceUrls = ListTools.convertToList(resourceFieldValue);
                // call implementation class "deleteResourceUrl" method
                this.insertResourceUrl(
                    declaredMethod,
                    String.valueOf(sourceFieldValue),
                    resourceField.type(),
                    resourceUrls);
              }
            });
   ...
   */
  @Override
  public void deleteResource(Method declaredMethod, Object[] param) throws MixmicroBootException {
    // get method declared ResourceField annotations
    List<ResourceField> resourceFields = ResourceFieldTools.getResourceFields(declaredMethod);
    resourceFields.stream()
        .forEach(
            resourceField -> {
              ...
              if (!ObjectUtils.isEmpty(sourceFieldValue)) {
                // call implementation class "deleteResourceUrl" method
                this.deleteResourceUrl(
                    declaredMethod, String.valueOf(sourceFieldValue), resourceField.type());
              }
            });
  }
   ...
   */
  @Override
  public void updateResource(Method declaredMethod, Object[] param) throws MixmicroBootException {
    // get method declared ResourceField annotations
    List<ResourceField> resourceFields = ResourceFieldTools.getResourceFields(declaredMethod);
    resourceFields.stream()
        .forEach(
  ...
  private void pushToMap(Method method, Map executeResultMap) {
    List<ResourceField> resourceFields = ResourceFieldTools.getResourceFields(method);
    executeResultMap.keySet().stream()
        .forEach(o -> push(method, resourceFields, executeResultMap.get(o)));
  }
        ...
        .forEach(
            resourceField -> {
              try {
                // source
                Field sourceField =
                    ResourceFieldTools.getField(objectClass, resourceField.source());
                // target
                Field resourceFieldInstance =
                    ResourceFieldTools.getField(objectClass, resourceField.name());

                // get source filed value
                Object sourceFieldValue = sourceField.get(object);
                // load resource urls
                List<String> resourceUrls =
                    loadResourceUrl(method, String.valueOf(sourceFieldValue), resourceField.type());
                ...
                throw new MixmicroBootException("Push resource fail.", e);
              }
            });
  }
}
