...
package xyz.vopen.framework.boot.plugin.resource.load.tools;
...
import xyz.vopen.framework.boot.plugin.resource.load.expression.ResourceSourceExpression;
import xyz.vopen.framework.boot.plugin.resource.load.loader.ResourceFieldLoader;
...
public class ResourceFieldTools {
  ...
  public static Field getField(Class objectClass, String fieldName) {
    // cache from memory
    Field sourceField =
        MixmicroBootResourceContext.getPushFieldFromCache(objectClass.getName(), fieldName);
    ...
    if (ObjectUtils.isEmpty(sourceField)) {
      ...
      // cache to memory
      MixmicroBootResourceContext.setPushFieldToCache(objectClass.getName(), fieldName, sourceField);
    }
    return sourceField;
  }
  ...
  /**
   * get method declared resource field annotation 1. get from cache 2. loading from method declared
   *
   * @param method method
  ...
  public static List<ResourceField> getResourceFields(Method method) {
    // get from cache
    List<ResourceField> resourceFields = MixmicroBootResourceContext.getResourceFieldFromCache(method);

    // loading from method declared
    if (ObjectUtils.isEmpty(resourceFields)) {
      resourceFields = loadMethodResourceFields(method);
      // set to cache
      MixmicroBootResourceContext.setResourceFieldToCache(method, resourceFields);
    }
    return resourceFields;
  }
}
