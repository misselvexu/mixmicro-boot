...
import xyz.vopen.framework.boot.plugin.message.push.model.MessagePushBody;
...
/**
 * Mixmicro Boot Message JPush Support
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 */
public class MixmicroBootMessagePushJiGuangServiceImpl extends MixmicroBootAbstractMessagePushServiceImpl {
  /** logger instance */
  static Logger logger = LoggerFactory.getLogger(MixmicroBootMessagePushJiGuangServiceImpl.class);

  public MixmicroBootMessagePushJiGuangServiceImpl(
   ...
   */
  @Override
  public void executePush(MessagePushBody messagePushBody) throws MixmicroBootException {
    try {
      // push client config
      ...
      JPushClient pushClient =
          new JPushClient(
              pushClientConfig.getMasterSecret(),
              pushClientConfig.getAppKey(),
              null,
   ...
   * setting platform
   *
   * @param messagePushBody Mixmicro Boot Message Push request body
  ...
  private PlatformNotification getIosNotificationMeta(MessagePushBody messagePushBody) {
    IosNotification.Builder builder =
        IosNotification.newBuilder()
            .setAlert(
                IosAlert.newBuilder()
                    .setTitleAndBody(
                        messagePushBody.getTitle(),
                        messagePushBody.getSubTitle(),
                        messagePushBody.getMessage())
                    .build())
            .setBadge(messagePushBody.getBadge())
            .setSound(messagePushBody.getSound());
    // extras param
  ...
  private PlatformNotification getAndroidNotificationMeta(MessagePushBody messagePushBody) {
    return AndroidNotification.newBuilder()
        .setTitle(messagePushBody.getTitle())
        .setAlert(messagePushBody.getMessage())
        .addExtras(messagePushBody.getExtras())
        .build();
  }
}
