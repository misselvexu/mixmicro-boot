package xyz.vopen.framework.boot.plugin.oauth;

import xyz.vopen.framework.boot.plugin.oauth.grant.MixmicroBootOauthTokenGranter;
import xyz.vopen.framework.boot.plugin.oauth.grant.DefaultApiBootOauthTokenGranter;
...
/**
 * Mixmicro Boot 集成Oauth2 相关配置实现
 *
 * @author <a href="mailto:iskp.me@gmail.com">Elve.Xu</a>
 */
public class MixmicroBootAuthorizationServerConfiguration extends AuthorizationServerConfigurerAdapter {
  ...
  private List<MixmicroBootOauthTokenGranter> mixmicroBootOauthTokenGranters;

  public MixmicroBootAuthorizationServerConfiguration(
      ObjectProvider<List<MixmicroBootOauthTokenGranter>> objectProvider) {
    this.mixmicroBootOauthTokenGranters = objectProvider.getIfAvailable();
  }

  /**
   * Configure secret encryption in the same way as Mixmicro Boot Security
   *
  ...
  public void configure(AuthorizationServerEndpointsConfigurer endpoints) throws Exception {
    endpoints
        ...
        // Mixmicro Boot custom token granter
  ...
  /**
   * Return all granters within oauth2 Contains custom
   *
   * @return TokenGranter
   */
  private List<TokenGranter> getDefaultTokenGranters() {
    ...
    if (!ObjectUtils.isEmpty(mixmicroBootOauthTokenGranters)) {
      mixmicroBootOauthTokenGranters.stream()
          .forEach(
              apiBootOauthTokenGranter ->
                  tokenGranters.add(
                      new DefaultApiBootOauthTokenGranter(
                          tokenServices,
