package xyz.vopen.framework.mybatis.pageable.dialect;
...
public abstract class AbstractDialect implements Dialect {
  ...
  /**
   * 添加分页参数映射 注意：参数的顺序直接映射参数的绑定，绑定占位符时从parameterMappings集合list内第一个依次匹配 如：sql : limit
   * ?,?，parameterMapping：{offset = 0,limit = 5} 最后匹配为 limit offset,limit，也就是limit 0,5
   *
  ...
  /**
   * 默认提供的获取分页的sql 支持：MySQL、Postgres、HSQL数据库 其他数据库重写该方法即可
   *
   * @param boundSql boundSql 对象
   * @param page 分页响应对象实例
   * @return 获取默认的分页sql
